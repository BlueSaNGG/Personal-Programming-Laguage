...
class RTError(Error):
    ...
    def generate_traceback(self):
        ...
        return 'Traceback the error calls (last call is the most recent call):\n' + result
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t' or self.current_char == '\n':
                self.advance()
            elif self.current_char in SYMBOL_DICT:
                tokens.append(Token(SYMBOL_DICT[self.current_char], pos_start = self.pos))
                self.advance()
            ...
            # String
                ...
                if self.current_char != '"':
                    return [], ExpectedCharError(pos_start, self.pos, '\'"\' at the end')
                else:
                    self.advance()
            else:
                #return error
...
class IfNode:
    ...
    def __repr__(self) -> str:
        return f'IF: {" | ELIF: ".join(f"{condition}->{expr}" for (condition, expr) in self.cases)} , ELSE: {self.else_case}'
...
class Parser:
    ...
    def list_expr(self):
        ...
        if self.current_tok.type != TT_LSQURE:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    f'Expected "["'
        ...
        if self.current_tok.type == TT_RSQURE:
            res.register_advancement()
            self.advance()
        else:
            ...
            if res.error: 
                return res.failure(
                    InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        'Expected "]", "BS", "IF", "FOR", "WHILE", "FUN", int, float, identifier, "+", "-", "(", "[" or "NOT"'
                    )
                )
    ...
    def atom(self):
        ...
        # LSQURE (expr (COMMA expr)*)? RSQURE
        elif tok.type == TT_LSQURE:
        ...
        return res.failure(
            InvalidSyntaxError(
                ...
                "Expected int, float, variable identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHILE', 'FUN' "
            )
        )
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'BS'):
            res.register_advancement()
            self.advance()
            # KEYWORD VARNAME = EXPRESSION
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_PLUS:
            ...
            result, error = left.subed_by(right)
            ...
            result, error = left.get_comparison_nq(right)
    ...
    def visit_IfNode(self, node, context):
        ...
        for condition, expr in node.cases:
            condition_value = res.register(self.visit(condition, expr))
            if res.error: return res
